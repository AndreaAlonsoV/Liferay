package com.iConsulto.miTablero.Reciver.interpreter;
...
public class IConsultoSocialActivityDescriptor {
			...
			try {
				if (_activity.getClassName().equals(KBArticle.class.getName())
					...
					String feedEntryTitleValue = getFeedEntryFieldValue(serviceContext.getLayoutFullURL(),
							MiTableroReciverPortletKeys.KNOWLEGE_BASE_URI, feedEntry.getTitle(),
							MiTableroReciverPortletKeys.KNOWLEGE_BASE_ID, kbArticle.getUrlTitle());
					...
					String feedEntryTitleValue = getFeedEntryFieldValue(serviceContext.getLayoutFullURL(),
							MiTableroReciverPortletKeys.MESSAGE_BOARDS_URI, feedEntry.getTitle(),
							MiTableroReciverPortletKeys.MESSAGE_BOARDS_MESSAGE_ID, String.valueOf(mbMessage.getMessageId()));
					feedEntry.setTitle(feedEntryTitleValue);

					String feedEntryBodyValue = getFeedEntryFieldValue(serviceContext.getLayoutFullURL(),
							MiTableroReciverPortletKeys.MESSAGE_BOARDS_URI, feedEntry.getBody(),
							MiTableroReciverPortletKeys.MESSAGE_BOARDS_CATEGORY_ID,
					...
					String docLibraryInstanceId = PropsUtil.get(MiTableroReciverPortletKeys.DOCUMENT_LIBRARY_INSTANCE_ID);
					String docLibraryFileId = MessageFormat.format(MiTableroReciverPortletKeys.DOCUMENT_LIBRARY_FILE_ID,
							docLibraryInstanceId);
					String docLibraryFolderId = MessageFormat.format(MiTableroReciverPortletKeys.DOCUMENT_LIBRARY_FOLDER_ID,
							docLibraryInstanceId);

					String feedEntryTitleValue = getFeedEntryFieldValue(serviceContext.getLayoutFullURL(),
							MiTableroReciverPortletKeys.DOCUMENT_LIBRARY_URI, feedEntry.getTitle(), docLibraryFileId,
							String.valueOf(dlFileEntry.getFileEntryId()));
					feedEntry.setTitle(feedEntryTitleValue);

					String feedEntryBodyValue = getFeedEntryFieldValue(serviceContext.getLayoutFullURL(),
							MiTableroReciverPortletKeys.DOCUMENT_LIBRARY_URI, feedEntry.getBody(), docLibraryFolderId,
	...
	private String getFeedEntryFieldValue(String fullUrl, String uri, String feedEntryFieldValue, String id,
		...
		int posHrefValue = feedEntryFieldValue.lastIndexOf(MiTableroReciverPortletKeys.HREF) + 6;
		String href = feedEntryFieldValue.substring(posHrefValue, feedEntryFieldValue.indexOf("\"", posHrefValue));

		if (href.indexOf(MiTableroReciverPortletKeys.RECYCLE_BIN) == -1) {
